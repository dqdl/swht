			...
			<div class="main">
				<div class="left">
					<n-space>
						<span>仓库名称</span>
						<n-input
							v-model:value="queryInfo.warehouseName"
							type="text"
							:style="{ width: '220px' }"
							placeholder="请输入仓库名称"
				...
				<n-form ref="formRef" :model="form_details" :rules="rules">
					<n-grid :cols="24" :x-gap="24">
						...
						<n-form-item-gi
							:span="12"
							label="仓库名称"
							path="user.warehouseName"
						>
							<n-input
								v-model:value="form_details.user.warehouseName"
								clearable
								:maxlength="formMaxLength"
								:disabled="disabled"
							/>
						</n-form-item-gi>
						<n-form-item-gi :span="12" label="仓库编号" path="user.warehouseId">
							<n-input
								v-model:value="form_details.user.warehouseId"
								clearable
								:maxlength="formMaxLength"
								:disabled="disabled"
							/>
...
// api
import api_house from '../../../axios/api/house/index';
...
const exportList = () => {
	...
	api_house
		...
		.then((res: any) => {
			...
			link.setAttribute('download', '仓库管理.xlsx');
	...
	formRef.value?.validate(async (error: any) => {
		if (!error) {
			...
			if (form_details.name === '新增') {
				...
				res = await api_house.newAdd({
					companyId: '131',
					params: {
						companyId: '131',
						ename,
						fileUrl,
						// "fileUrl[0].exactType": 'png',
						// "fileUrl[0].url": fileList.length?fileList[0].file.url:'xxx',
						point: {
							id: '112',
...
let postUrl: any = api_house.uploadImg;
...
const rowEdit = async (row: any, index: number) => {
	...
	showModal_newAdd.value = true;
	// const res: any = await api_car.getImage(row.id)
	fileList.length = 0;
	if (row.fileUrl && row.fileUrl.length) {
		row.fileUrl.forEach((item: any) => {
			fileList.push({
...
const rowDelete = (row: any, index: number) => {
	...
	form_details.user.warehouseId = row.warehouseId;
};
// 确认删除
const submitDelete = async () => {
	const id = form_details.user.warehouseId;
	const res: any = await api_house.delete(id);
	if (res.code === 200) {
		getList();
	} else {
...
// 查询
const query = () => {
	getList({ warehouseName: queryInfo.warehouseName });
};
// 重置
const reset = () => {
	queryInfo.warehouseName = '';
	getList();
};
// 分页翻页
...
const getList = async (data?: any) => {
	...
	const res: any = await api_house.getList(params);
	console.log(res);
	if (res.code === 200) {
		// page
