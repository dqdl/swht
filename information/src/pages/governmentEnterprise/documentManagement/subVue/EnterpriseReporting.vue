...
let chcheDatas: any = store.state.postArticle.draftDatas;
		...
		_.children.map((item: any, index: number) => {
			...
			if (item.children.length !== 0) {
				obj = {
					...
					value: item.deptName,
					// key: item.id,
					key: `${item.deptName},${item.id},${item.parentName}`,
				...
				obj = {
					...
					value: item.deptName,
					// key: item.id,
					key: `${item.deptName},${item.id},${item.parentName},root`,
					parent: item?.parentName,
					children: tersvice(item),
					tag: false,
				};
