	...
	<n-message-provider>
		<div class="container">
			...
			<n-form
				...
				:rules="rules"
			>
				<n-form-item label="通知类型:" path="inputValue">
					<n-input v-model:value="model.inputValue" placeholder="请输入发文标题" disabled />
				</n-form-item>
				<n-form-item label="通知标题:" path="textareaValue">
					<n-input v-model:value="model.textareaValue" placeholder="请输入通知标题" />
				</n-form-item>
				<n-form-item label="通知主题:" path="multipleSelectValue">
					<n-input v-model:value="model.multipleSelectValue" placeholder="请输入通知主题" />
				</n-form-item>
				<n-form-item label="通知正文:" path="switchValue">
					<n-input v-model:value="model.switchValue" type="textarea" placeholder="请输入通知正文" />
				</n-form-item>
				...
				<n-form-item label="通知对象" path="onlineselect" :show-feedback="false">
					<span class="online_select">
						当前已选择
						<p>{{ selectValueEdid }}</p>条
					</span>
					<n-select placeholder="请输入通知对象" :options="reportObj" @update:value="henld($event)" />
				</n-form-item>
			</n-form>
			<!-- footer部分 -->
			<div class="footer">
...
console.log(store.state.postArticle.draftDatas, 'oooiiiii');
...
function confirmSelect() {
	...
	functionalIds.value =
		...
		dataList.map((item: any, index: number) => {
			if (item.key.split(',')[3] !== '' && item.key.split(',')[3]) {
				// 工业区
				return item.key.split(',')[1]; // 拿到id值形成ids数组
			} else {
				return item.key.split(',')[1]; // 功能区
			}
		});
...
function footerClick(value: any) {
	switch (value) {
		case '确认派发':
			console.log(model.value, '88888888');
			if (
		...
		case '取消发文':
			...
			model.value.textareaValue = null;
		...
		_.children.map((item: any, index: number) => {
			...
			if (_.children.length !== 0) {
				...
				obj = {
					...
					value: item.namess,
					// key: item.id,
					key: `${item.namess},${item.id},${item.parentname},root`,
					parent: item?.parentname,
					children: tersvice(item),
				};
			}
