    ...
    <div class="userManagement">
                ...
                <div class="left">
                    ...
                    <div style="margin-top: 20px;">
                        <n-space>
                            ...
                            />
                            <address-vue :de="false" ref="lookAddress" />
                        </n-space>
                        <!--  -->
                        <n-space>
        ...
        >
            ...
            <n-form ref="formRef" :model="form_details" :rules="rules">
                ...
                <n-form-item label="企业" path="user.c">
                    <n-input
                ...
                <!-- address -->
                <n-form-item label="户籍地址 " path="user.role">
                    <address-vue
                        :dataAddress="form_details.user.permanentAddress"
                        ref="permanentAddress"
                    />
                </n-form-item>
                <n-form-item label="现居住地址" path="user.role">
                    <address-vue
                        :dataAddress="form_details.user.currentResidentialAddress"
                        ref="currentResidentialAddress"
                    />
                </n-form-item>
            </n-form>
            <template #footer>
...
// vue
import { ref, reactive, h, onMounted, VueElement } from 'vue'
// naive_ui
...
const form_details = reactive({
    ...
    user: {
        ...
        phone: '',
        code: '',
        enterpriseId: '40',//企业id
        enterpriseName: '邵武市腾越竹制品有限公司',//企业名称
...
const dealQueryData = () => {
    ...
    if (addressInfo.name == '户籍') {
        params.permanentAddress = lookAddress.value.addressStr()
    } else {
        params.currentResidentialAddress = lookAddress.value.addressStr()
    }
...
// 详情/编辑弹框打开时
const open = (row: any) => {
    const { name, account, deptName, job, role, phone, code, currentResidentialAddress, permanentAddress } = row
    ...
    form_details.user.currentResidentialAddress = currentResidentialAddress
    form_details.user.permanentAddress = permanentAddress
...
const newAdd = () => {
    ...
    Object.keys(form_details.user).forEach(item => {
        if (item == 'enterpriseName' || item == 'enterpriseId') {
...
const permanentAddress: any = ref(null);
const currentResidentialAddress: any = ref(null);
...
const getNewAddForm = (e: any) => {
    ...
    dragDivDom.validate((errors: any) => {
        if (!errors) {
            form_details.user.permanentAddress = permanentAddress.value.addressStr()
            form_details.user.currentResidentialAddress = currentResidentialAddress.value.addressStr()
        ...
        .then((res: any) => {
            if (res.code === 200) {
                ...
                res.data.records.forEach((item: any, index: number) => {
                    const { name, account, createTime, phone, deptName, job, code, currentResidentialAddress, permanentAddress } = item
