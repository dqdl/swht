    ...
    <div class="changepwd_box">
      ...
      <div>
        <div class="change_phone user_input">
          ...
          <input v-model="user.account" type="text" :placeholder="changepwdData.placeholder" />
        </div>
        <div class="change_pwd user_input">
          <div class="change_pwd_img user_input_img">
            <img src="@/assets/images/login/user_pwd.png" />
          </div>
          <input v-model="user.password" :type="pwdShow.password" placeholder="请输入密码" />
          <div class="change_pwd_show_img user_input_img user_input_img_none csspointer">
            <img @click="isShow(1)" src="@/assets/images/login/user_pwd_show.png" />
          </div>
        </div>
        ...
        <div class="change_new_pwd1 user_input">
          ...
          />
          <div class="change_pwd_show_img user_input_img user_input_img_none csspointer">
            <img @click="isShow(2)" src="@/assets/images/login/user_pwd_show1.png" />
          </div>
        ...
        <div class="change_new_pwd2 user_input">
          ...
          />
          <div class="user_pwd_show_img user_input_img user_input_img_none csspointer">
            <img @click="isShow(3)" src="@/assets/images/login/user_pwd_show1.png" />
          </div>
        </div>
        <div class="user_smb_btn">
          <n-button @click="changePwdBtn" color="#1e9fff" type="success">确定重置</n-button>
        </div>
      </div>
    </div>
  </div>
...
// 验证
let { verificauser, verificapwd, AnalyzePasswordSecurityLevel, verificanewpwd } = verification();
// 数据
...
const isShow = (num: number) => {
  if (num === 1) {
    ...
    pwdShow.newPassword = pwdShow.newPassword == "password" ? "text" : "password";
  } else {
    pwdShow.newTwoPassword = pwdShow.newTwoPassword == "password" ? "text" : "password";
  }
}
// 密码修改提交
const changePwdBtn = () => {
