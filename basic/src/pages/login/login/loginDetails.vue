<template>
    ...
    <div class="user_pwd user_input">
        ...
        <input :type="isType" v-model="user.password" placeholder="请输入密码" />
        <div class="user_pwd_show_img user_input_img user_input_img_none">
            <img @click="pwdShow" class="csspointer" :src="imgShow.src" />
        </div>
    </div>
    ...
    <div class="user_smb_btn">
        <n-button :loading="loading" @click="smbLogin" color="#1e9fff" type="success">登录</n-button>
    </div>
    <!-- 跳转其他 -->
    <div class="jump_other">
        <div @click="registeuser" class="user_register csspointer">账号注册</div>
...
// 获取验证码
const getCode = () => {
    loginHome.getcode().then((res: any) => {
        if (res.code == 200) {
