			...
			<div class="query">
				<div class="left">
					...
					<div style="width: 100%">
						<n-space>
							<span>巡检企业名称</span>
							<n-input v-model:value="queryInfo.enterpriseName" type="text" :disabled="true"
								:style="{ width: '800px' }" placeholder="请输入巡检企业名称" clearable />
						</n-space>
					</div>
					<div>
						<n-space>
							<span>计划名称</span>
							<n-input v-model:value="queryInfo.planName" type="text" :style="{ width: '300px' }"
								placeholder="请输入计划名称" clearable />
						</n-space>
						<n-space>
							<span>计划编号</span>
							<n-input v-model:value="queryInfo.planNumber" type="text" :disabled="true"
								:style="{ width: '300px' }" placeholder="请输入计划编号" clearable />
						</n-space>
					</div>
					<!-- time -->
					<div>
						<n-space>
							<span>关联政企互动发文</span>
							<n-select v-model:value="queryInfo.sendName" :options="industryList"
								placeholder="请选择关联政企互动发文" :style="{ width: '300px' }" @update:value="getSendId" />
						</n-space>
					</div>
					<!-- <div>
						<n-space>
		...
		<div class="list">
			...
			<div class="tableList">
				<n-config-provider :locale="zhCN" :date-locale="dateZhCN">
					<n-data-table ref="table" :remote="true" :bordered="false" :max-height="500" :virtual-scroll="true"
						:columns="tableHead" :data="tableData" :loading="tableLoading"
			...
			<n-card :bordered="false">
				...
				<n-grid :cols="24" :x-gap="24">
					<n-form-item-gi :span="24" label="任务编号" path="user.facilityName">
						<n-input v-model:value="form_details.user.taskSerial" type="text" placeholder="请输入任务编号"
							:disabled="true" clearable />
					</n-form-item-gi>
					<n-form-item-gi :span="24" label="任务名称" path="user.facilityName">
						<n-input v-model:value="form_details.user.taskName" type="text" placeholder="请输入任务名称"
							:disabled="form_details.name == '详情'" clearable />
					</n-form-item-gi>
					<n-form-item-gi :span="12" label="巡查点位" path="user.facilityName">
						<!-- <n-input
							...
							clearable
						/>-->
						<n-select v-model:value="form_details.user.checkPoint" :options="repositoryList"
							placeholder="请选巡查点位" :disabled="form_details.name == '详情'" />
					</n-form-item-gi>
					<n-form-item-gi :span="12" label="期限完成时间" path="user.facilityName">
						<n-config-provider :style="{ width: '100%' }" :locale="zhCN" :date-locale="dateZhCN">
							<n-date-picker v-model:value="form_details.user.completeTime"
					...
					<n-form-item-gi :span="24" label="检查内容" path="user.facilityName">
						<n-input v-model:value="form_details.user.checkContent" type="textarea" :autosize="{
							minRows: 3,
							maxRows: 5,
						}" placeholder="请输入检查内容" clearable :disabled="form_details.name == '详情'" />
					</n-form-item-gi>
					<n-form-item-gi :span="24" label="巡检人员" path="user.facilityName">
						<!-- <n-input
							...
							clearable
						/>-->
						<n-select v-model:value="form_details.user.userId" multiple :options="userList"
							@update:value="getUserName" :disabled="form_details.name == '详情'" placeholder="选择巡检人员"
							:style="{ width: '300px' }" />
					</n-form-item-gi>
				</n-grid>
				<template #footer>
					<n-space>
						<n-button v-if="form_details.name !== '详情'" type="info" @click="sure">保存</n-button>
...
const getUserName = (e: any, option: any) => {
	form_details.user.checkMan = option[0].label
}
// 获取
...
const numberProduct1 = () => {
	Api1.numberProduct().then((res: any) => {
		queryInfo.planNumber = res.data.jhnum;
	});
};
// 获取随机任务编号
const numberProduct2 = () => {
	Api1.numberProduct().then((res: any) => {
		form_details.user.taskSerial = res.data.RWBH;
	});
};
let sendId: any = '';
// 获取发文ID
...
// 发布
const Release = async () => {
	let data: any = await Api1.updatePlan({ planStatus: "0", ...queryInfo });
